...
function Resume() {
    return (
        <main className="p-8 bg-slate-50 dark:bg-slate-800 rounded-3xl mt-2 md:mt-0 mb-10 md:mb-0">
            ...
            <div className="flex gap-1 md:gap-1 lg:gap-x-4 lg:gap-y-3 lg:flex-wrap justify-between lg:justify-start overflow-x-auto">
                {knowledges.map((item, index) => (
                    <div key={index} className="bg-red-400 dark:bg-slate-700 px-5 py-1 rounded-3xl font-medium text-gray-50 min-w-max">{item.label}</div>
                ))}
            </div>
        </main>
    );
